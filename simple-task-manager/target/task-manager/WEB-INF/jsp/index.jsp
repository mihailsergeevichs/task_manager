...
<div class="container">
        <c:choose>
            <c:when test="${unchecked.size() == 0}">
                <p> <h1 class="alert-info"> Your task list is empty!</h1> </p>
            </c:when>
            <c:otherwise>
                <h1 class="active">Your current unchecked tasks</h1>
                <table class="table table-bordered">
                    <tbody>
                    <tr>
                        <th>Label</th><th>Date</th><th>Description</th><th>Category</th>
                    </tr>
                    <c:forEach items="${unchecked}" var="task">
                        <tr>
                            ...
                            <td>${fn:substring(task.description, 0, 50).concat("...")}</td>
                            <td>${task.category}</td>
                        </tr>
                    </c:forEach>
                    </tbody>
